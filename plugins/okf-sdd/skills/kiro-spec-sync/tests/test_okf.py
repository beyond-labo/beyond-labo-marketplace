...
class CheckerTests(unittest.TestCase):
    ...
    def test_external_bookkeeping_names_are_real_dependencies(self):
        for name in ('docs/index.md', 'contracts/spec.json', 'logs/log.md'):
            with self.subTest(name=name):
                dep = self.root / name
                dep.parent.mkdir(exist_ok=True)
                dep.write_text('contract v1')
                self.doc.write_text('---\ntype: Requirements\nkiro:\n  depends_on: [' + name + ']\n---\nBody\n')
                self.snapshot()
                self.assertIn(name, json.loads(self.state.read_text())['freshness']['checked_inputs'])
                before = self.state.read_bytes()
                dep.write_text('breaking contract v2')
                code, result = self.runcli('check', '--feature', 'demo')
                self.assertEqual(code, 1)
                self.assertIn('NEEDS_REVIEW', str(result))
