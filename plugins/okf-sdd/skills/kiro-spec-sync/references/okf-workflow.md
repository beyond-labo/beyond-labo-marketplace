...
プロファイルと Python 環境の選択は [利用環境](setup.md) に従う。
同梱の形式定義は [okf-profile.md](../assets/settings/okf-profile.md)。
以下のコマンドはリポジトリルートから実行する。
Python 3 と `scripts/requirements.txt` に記した PyYAML が必要。既存環境を使い、依存追加が必要なら実行環境の許可手順に従う。

```sh
python3 "$SKILL_DIR/scripts/okf.py" check --root . --feature FEATURE
```
...
```sh
python3 "$SKILL_DIR/scripts/okf.py" snapshot --root . --feature FEATURE --reviewed
python3 "$SKILL_DIR/scripts/okf.py" index --root . --feature FEATURE
```
