...
`.kiro/specs/<feature>/spec.json` と bundle 内の生成 index、log は入力ハッシュに含めず、自己参照や帳簿更新による無限の変更検出を避ける。
bundle 外の明示的な出典・契約依存は、同じファイル名でもハッシュへ含める。
...
- [OKF v0.2 公式仕様](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md)（2026-09-10確認）。型、出典、来歴、状態、索引の基礎。
- [MADR](https://adr.github.io/madr/)（2026-09-10確認）。重要判断を構造化する本文の参考。
