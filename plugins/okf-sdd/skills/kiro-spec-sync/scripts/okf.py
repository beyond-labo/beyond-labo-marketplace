...
class Audit:
    ...
    def bookkeeping(self, path):
        specs = self.root / '.kiro/specs'
        steering = self.root / '.kiro/steering'
        if path.name in ('index.md', 'log.md'):
            return path.is_relative_to(specs) or path.is_relative_to(steering)
        return path.name == 'spec.json' and path.parent.parent == specs
    ...
    def inputs(self, directory):
        ...
        while todo:
            path = todo.pop()
            if path in seen or self.bookkeeping(path): continue
            seen.add(path)
