...
.kiro の仕様駆動開発で、承認済み仕様のタスクを実装・検証し、実装で得た知見を現行仕様へ同期する。タスク指定時はその範囲、未指定時は未完了タスクを扱う。
...
```text
$kiro-impl notifications 1.1 承認済みタスクを実装し、必要な検証と仕様同期まで進めてください。
```
...
[SKILL.md](https://github.com/beyond-labo/beyond-labo-marketplace/blob/main/plugins/okf-sdd/skills/kiro-impl/SKILL.md)
